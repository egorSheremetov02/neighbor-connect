        ...
        <Toolbar>
          ...
          {isMobile ? (
            <>
              ...
              >
                {menuItems.map((item) => (
                  <MenuItem key={item.name} onClick={() => { setActiveComponent(item.name.toLowerCase()); handleClose(); }}>
                    {item.name}
                  </MenuItem>
                ))}
              </Menu>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2 }}>
              {menuItems.map((item) => (
                <Button
                  ...
                  sx={{ backgroundColor: item.name === 'Login' ? 'blue' : 'transparent', color: item.name === 'Login' ? 'white' : '#1976d2', width: '100px', height: '40px' }}
                >
                  {item.name}
                </Button>
              ))}
