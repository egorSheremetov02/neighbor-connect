    ...
    with SessionLocal() as session:
        with session.begin():
            ...
            if profile_request.email and profile_request.email != user.email:
                existing_user = session.query(User).filter_by(email=profile_request.email).first()
                if existing_user:
                    raise HTTPException(status_code=409, detail="Email already in use")

            if profile_request.login and profile_request.login != user.login:
                existing_user = session.query(User).filter_by(login=profile_request.login).first()
                if existing_user:
                    raise HTTPException(status_code=409, detail="Login already in use")

            if profile_request.phone_number and profile_request.phone_number != user.phone_number:
                existing_user = session.query(User).filter_by(phone_number=profile_request.phone_number).first()
                if existing_user:
            ...
            user.permanent_address = profile_request.permanent_address or user.permanent_address
            user.current_address = profile_request.current_address or user.current_address
            ...
            user.bio_description = profile_request.bio_description or user.bio_description
